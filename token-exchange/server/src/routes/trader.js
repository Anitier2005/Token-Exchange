...
const router = Router();
router.use(authRequired, roleRequired('trader'));
...
router.get('/account', async (req, res, next) => {
  try {
    ...
    // 占用保证金 = Σ 持仓价 × 手数 × 保证金比例
    const pos = await query(
      `SELECT p.volume, p.avg_price FROM positions p WHERE p.user_id = $1`, [req.user.id]
    );
    ...
    res.json({ balance, usedMargin: +used.toFixed(2), available: +(balance - used).toFixed(2) });
  } catch (e) { next(e); }
});
...
router.post('/orders', async (req, res, next) => {
  try {
    ...
    if (!['long', 'short'].includes(side)) return res.status(400).json({ error: '方向不合法' });
    if (!vol || vol <= 0 || !Number.isInteger(vol)) return res.status(400).json({ error: '手数必须为正整数' });

    const cfg = (await query('SELECT * FROM exchange_config WHERE id = 1'))[0];

    // 交易时间
    if (!isWithinTradingHours(cfg)) {
      return res.status(400).json({ error: `当前不在交易时间内（${cfg.trading_start}-${cfg.trading_end}）` });
    }
    // 手动熔断
    if (cfg.manual_halted) {
      return res.status(400).json({ error: `交易所已暂停交易：${cfg.halt_reason || '手动熔断'}` });
    }

    const futures = await query('SELECT * FROM futures WHERE id = $1', [Number(futureId)]);
    const f = futures[0];
    if (!f) return res.status(404).json({ error: '期货不存在' });
    if (f.status !== 'active') return res.status(400).json({ error: '该期货当前不可交易' });
    if (f.halted) return res.status(400).json({ error: `该期货已熔断：${f.halt_reason}` });

    // 交易门槛
    if (vol < f.min_volume) {
      return res.status(400).json({ error: `低于交易门槛，最小手数为 ${f.min_volume}` });
    }
    ...
    if (last.length) {
      ...
      if (elapsed < cfg.trade_interval_sec) {
        return res.status(429).json({ error: `交易频率超限，每 ${cfg.trade_interval_sec} 秒最多一笔` });
      }
    }
    ...
    if (closeVol > 0) {
      const diff = oppo === 'short'
        ? Number(oppoPos.avg_price) - price   // 平空：开仓价 - 平仓价
        : price - Number(oppoPos.avg_price);  // 平多：平仓价 - 开仓价
    ...
    if (oppoPos) used -= oppoPos.volume * Number(oppoPos.avg_price) * marginRatio; // 该仓位将被部分/全部释放
    const released = oppoPos
      ? +(closeVol * Number(oppoPos.avg_price) * marginRatio).toFixed(6) : 0;
    const need = +(addMargin + fee + tax).toFixed(6);
    const available = +(balance - used + released).toFixed(6);
    if (available < need) {
      return res.status(400).json({ error: `保证金不足：需 ${need}，可用 ${available}` });
    }
