...
// 行情引擎：生成价格随机游动 tick、撮合限价挂单、触发熔断、广播 L1/L2 数据
class MarketEngine extends EventEmitter {
  constructor() {
    super();
    this.state = new Map(); // futureId -> { price, dayOpen, prevClose, basePrice, ticks, date, halted, haltReason }
    this.config = null;
    this.timer = null;
    this.currentDate = null;
    this.tickCount = 0;      // 运营指标：累计 tick 数
    this.matchCount = 0;     // 运营指标：累计撮合成交数
    this.lastTickAt = 0;
  }
